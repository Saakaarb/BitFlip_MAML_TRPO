...
class CostModel():
	...
	def get_temporal_cost(self, state, prevstate):
		...
		# reward = 2*progress - dist/20 - np.logical_or(state.Vx<self.vmin, state.Vx>self.vmax) - np.sqrt(state.Vx)*np.abs(state.δ)
		# reward = 2*progress - dist/20 - np.logical_or(state.Vx<self.vmin, state.Vx>self.vmax) - 5*(state.Vx>self.vmin)*np.abs(state.δ)
