...
def weighted_mean(tensor, lengths=None):
    ...
    if tensor.dim() < 2:
        raise ValueError('Expected tensor with at least 2 dimensions (trajectory_length x batch_size), got {0}D tensor.'.format(tensor.dim()))
...
def detach_distribution(pi):
    if isinstance(pi, Independent):
        distribution = Independent(detach_distribution(pi.base_dist),pi.reinterpreted_batch_ndims)
        ...
        raise NotImplementedError('Only `Categorical`, `Independent` and `Normal` policies are valid policies. Got `{0}`.'.format(type(pi)))
    return distribution
